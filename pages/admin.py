                ...
                if fields[0]:
                    ...
                    if admission_ticket:
                        if fields[1] == "初階":
                            # 學科
                            exam_s_filter = {
                                "admission_ticket_no" : admission_ticket,
                                "exam_id" : "SE0917"
                            }
                            exam_s_data = {}

                            if fields[3] or fields[3] == 0:
                                exam_s_data["exam_grade"] = str(fields[3])
                                QueryExamsLogs.exams_update(exam_s_filter,exam_s_data)
                            ...
                            if fields[4] or fields[4] == 0:
                                exam_p_data["exam_grade"] = str(fields[4])
                                QueryExamsLogs.exams_update(exam_p_filter,exam_p_data)
                        elif fields[1] == "進階":
                            # 學科
                            exam_s_filter = {
                                "admission_ticket_no" : admission_ticket,
            ...
            if s.exam_grade != "" and s.exam_grade != "-" and s.exam_grade != "缺考":
                ...
                # 2021 test 術科
                    ...
                    if float(s.exam_grade) >= 70:
                        ...
                        test_status = "FAIL"
                # 2022 test 初階學科
                elif s.exam_id == 'SE0917':
                    if float(s.exam_grade) >= 70:
                        test_status = "PASS"
                    else:
                        test_status = "FAIL"
                # 2022 test 進階學科
                elif s.exam_id == 'SA0917':
                    if float(s.exam_grade) >= 85:
                        test_status = "PASS"
                    else:
                        test_status = "FAIL"
                # 2022 test 術科
                elif s.exam_id == 'PE0917' or s.exam_id == 'PA0917':
                    if float(s.exam_grade) >= 70:
                        test_status = "PASS"
                    else:
                        test_status = "FAIL"
