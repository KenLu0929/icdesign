...
def ic_report(request):
    ...
    #print(exams_logs_filter)

    exam_logs = QueryExamsLogs.exams_get(exams_logs_filter, False)
    #print(exam_logs)

    if isinstance(exam_logs, list):
        for a in exam_logs:
            exams_id_list.append(a.get("exam_id"))
            final_output["admission_ticket_no"] = a.get("admission_ticket_no")
    else:
        exams_id_list.append(exam_logs["exam_id"])
        final_output["admission_ticket_no"] = exam_logs["admission_ticket_no"]
    ...
    if isinstance(exams_2nd, list):
        for b in exams_2nd:
            exam_name_list[b.get("exam_id")] = b.get("exam_name")
            exam_level_list[b.get("exam_id")] = b.get("exam_level")
    else:
        exam_name_list[exams_2nd["exam_id"]] = exams_2nd["exam_name"]
        exam_level_list[exams_2nd["exam_id"]] = exams_2nd["exam_level"]
    ...
    if isinstance(exam_logs, list):
        for c in exam_logs:
            ...
            # print(exams_log_dict)
            # print(exams_log_dict["exam_grade"])
            # print(type(exams_log_dict["exam_grade"]))

            if exams_log_dict["exam_id"] == "SE0917":
                pass_score = 70.0
            elif exams_log_dict["exam_id"] == "PE0917":
                pass_score = 70.0
            elif exams_log_dict["exam_id"] == "SA0917":
                pass_score = 85.0
            elif exams_log_dict["exam_id"] == "PA0917":
                pass_score = 70.0
            ...
            if exams_log_dict["exam_grade"] != "-":
                ...
                exams_log_dict["exam_grade"] = float(-1)
            exams_logs_final.append(exams_log_dict)
    else:
        exams_log_dict = {
            "exam_id": exam_logs["exam_id"],
            "exam_name": exam_name_list.get(exam_logs["exam_id"]),
            "exam_level": exam_level_list.get(exam_logs["exam_id"]),
            "exam_grade": exam_logs["exam_grade"],
        }
        ...
        if exams_log_dict["exam_grade"] != "-":
            exam_score = float(exam_logs["exam_grade"])
            # print(exam_score)
            if exam_score >= pass_score:
                # print("PASS")
                exams_log_dict["exam_finish"] = True
...
def ic_admission_ticket(request):
    ...
    if isinstance(exam_logs, list):
        ...
        exams_id_2nd.append(exam_logs["exam_id"])
        exams_admission_ticket.append(exam_logs["admission_ticket_no"])
    ...
    if isinstance(exams_2nd, list):
        ...
        exams_date.append(str(exams_2nd["exam_start_time"]).split("T")[0])
...
def download_file_2022_exam_seat(request):
    # fill these variables with real values
    fl_path = 'pages/IC_111_exam_seat.pdf'
    filename = 'IC_111_exam_seat.pdf'
    fl = open(fl_path, 'rb')
    mime_type, _ = mimetypes.guess_type(fl_path)
    response = HttpResponse(fl, content_type=mime_type)
    response['Content-Disposition'] = "attachment; filename=%s" % filename
